...
Quantitative evaluation is presented in~Table~\ref{tbl:PSNR_NO_MASK} and a graphical visualization in ~Figure~\ref{fig:error_psnr_errorbar}. The PSNR is calculated for the entire image without masking. Note that the proposed method outperforms the others. To support it, we also calculated the Two-sample t-Test for the table's scores. The differences are statistically significant (p-values~$<0.005$) for all pairwise comparisons between the results of our method and the others.
...
Dice scores for all tested methods and sampling factors are presented in~Table~\ref{tbl:DICE}. As fore previous tests, our method achieves the best scores for all brain tissues and all sampling factors. {\color{red} Statistical significant values (~~) found for [1,2,3,4].}
...
Combining both adversarial and fidelity loss in the DNN training, the method suggested provides high PSNR reconstructions for large and diverse brain MRI dataset acquired by different MRI machines (1.5T and 3.T) and different imaging protocols.

Clinical usability of the reconstructed data plays a key role in the quality analysis performed as part of this research. We choose to test it by using standard tissue segmentation measures (such as Dice and MHD), as we believe that compatibility of the extracted brain regions with those obtained by using the original fully-sampled MRIs indicate fine detail reconstructions which have apparent diagnostic significance.

Until recently, CS techniques were considered the state-of-the-art for MRI reconstruction.
