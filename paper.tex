...
\end{equation}
where $L\left(\cdot\right)$ is a loss function. L2 loss is widely used for natural image reconstruction. However, as mentioned in \cite{pathak2016context}, L2 minimization provides a blurry solution, results in a loss of fine details. In our case, where spatial frequencies in the form of k-space are considered, using this loss alone may be inappropriate, resulting in poor reconstruction. To address this problem, we proposed using in addition an adversarial loss, based on GAN.
...
In equilibrium, the generator $G$ imitates the true data, which is in our the missing k-space samples. The generator loss is a linear combination of L2 fidelity loss and an adversarial loss as follows:
...
The generator input is a two-channel representing the real and the imaginary parts of the partially sampled k-space image, $s_{p}$. Each missing sample $\left(i,j\right)$ is initialized by uniform i.i.d. noise as follows: 
\begin{equation}
...
\end{equation}
where $G_{out}$ is the generator output. The discriminator's two-channel (real and imaginary) input is either the reconstructed or the true MR image obtained by IFFT of the generator's estimated or the original fully-sampled k-space data, respectively. By that, we are incorporating the IFFT into the network performing end-to-end optimization. The discriminator's output defines the WGAN loss.
