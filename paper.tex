...
The training data consists of 500 $3D$ brain MRI (T1-weighed) scans of different patients from the IXI dataset\footnote{\url{http://brain-development.org/ixi-dataset/}}. The data was acquired by three MR machines, Philips 1.5T, 3T and GE 1.5T using different imaging protocols.
...
Unlike our preliminary work~\cite{shitrit2017accelerated}, which employed 1D Cartesian sampling mask along the phase axis, here, we perform k-space sampling using $2D$ random Gaussian mask. By this, we exploit the sparsity in both phase and frequency dimensions and are therefore able to accommodate higher sampling factors (Figure~\ref{fig:sampling_masks}). MRI reconstruction is performed using 40\%, 25\% and 16.6\% of the original k-space data.
