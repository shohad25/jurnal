...
Compressed manifold learning based on Laplacian eigenmaps, has been applied to partially in~\cite{usman2014compressive} for respiratory motion estimation and in~\cite{bhatia2015fast} for cardiac MRI reconstruction. 
...
The spatio-temporal characteristics of sequential MRI data was recently exploited using recurrent neural network (RNN) by~\cite{qin2017convolutional}
Another interesting approach, based on recurrent neural network (RNN) for temporal MRI reconstruction has been recently suggested by~\cite{qin2017convolutional}. All these works address the reconstruction problem in the image domain rather than the k-space domain.
...
PSNR and SSIM are the most commonly used measures for evaluate image reconstruction quality. PSNR measures the Mean Squared Error (MSE) between the fully-sampled MR image and the reconstructed image. Therefore, this measure is not a sufficient metric for edges and general objects shape. For example, a model can provides a good reconstruction in the sense of PSNR but with blurry edges and loss of anatomical fine details. This leads to a poor segmentation, which used for medical diagnostic.
